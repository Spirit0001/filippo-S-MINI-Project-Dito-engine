...
open class RectangleRotated(position: Vector?, var width: Float, var height: Float, color: Int) :
    GameObject(
        position!!
    ), Caged {
    // Set up the paint.
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        val left = position.x - (width / 10)
        val top = position.y - (height /1)
        val right = position.x + (width / 10)
        val bottom = position.y + (height / 11)
